...
namespace Hale_Marketing_International
{
    public partial class PaymentReceiptWindow : Window
    {
        ...
        private int _editingVoucherDbId = -1;
        ...
        // ── Constructor ─────────────────────────────────────────────────────────
        public PaymentReceiptWindow()
        {
            InitializeComponent();
            dgPaymentReceipts.ItemsSource = _receiptItems;
            EnsureTable();
            LoadParties();
            LoadNextVoucherNo();
            DtPaymentDate.SelectedDate = DateTime.Today;
        }
        ...
        private void EnsureTable()
        {
            try
            {
                ...
                con.Open();
                new SQLiteCommand(@"CREATE TABLE IF NOT EXISTS PaymentReceipt (
                ...
                {
                    "ALTER TABLE PaymentReceipt ADD COLUMN AttachName TEXT",
                    "ALTER TABLE PaymentReceipt ADD COLUMN AttachData BLOB"
                })
                { try { new SQLiteCommand(sql, con).ExecuteNonQuery(); } catch { } }
            }
            catch (Exception ex) { MessageBox.Show("EnsureTable (PaymentReceipt): " + ex.Message); }
        }
        ...
        private void LoadNextVoucherNo()
        {
            try
            {
                ...
                using var cmd = new SQLiteCommand(
                    "SELECT IFNULL(MAX(CAST(SUBSTR(VoucherNo,4) AS INTEGER)),0)+1 FROM PaymentReceipt", con);
                TxtVoucherNo.Text = "PR-" + Convert.ToInt64(cmd.ExecuteScalar()).ToString().PadLeft(6, '0');
            }
            catch { TxtVoucherNo.Text = "PR-000001"; }
        }
        ...
        private static void OpenBytesWithDefaultApp(byte[] data, string filename)
        {
            try
            {
                ...
                string tmp = Path.Combine(Path.GetTempPath(), $"hmi_pr_{Guid.NewGuid():N}{ext}");
        ...
        private void BtnAdd_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                _receiptItems.Add(new PaymentReceiptItem
                {
        ...
        {
            TxtTotalAmount.Text = _receiptItems.Sum(x => x.Amount).ToString("N2");
        ...
        private void BtnSave_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (_editingVoucherDbId > 0)
                    new SQLiteCommand("DELETE FROM PaymentReceipt WHERE VoucherNo=@v",
                        con, tran)
                ...
                foreach (var item in _receiptItems)
                {
                    ...
                    cmd.Parameters.AddWithValue("@Date", DtPaymentDate.SelectedDate ?? DateTime.Now);
        ...
        private void BtnPrint_Click(object sender, RoutedEventArgs e)
        {
            ...
            var doc = BuildFlowDocument(
                TxtVoucherNo.Text,
                DtPaymentDate.SelectedDate?.ToString("yyyy-MM-dd") ?? DateTime.Today.ToString("yyyy-MM-dd"),
                partyName, phone, address, _receiptItems.ToList(), total);
            ...
            try
            {
                ...
                using var cmd = new SQLiteCommand(@"
                    SELECT pr.VoucherNo,
                           ...
                           MIN(pr.Id)               AS FirstId
                    FROM PaymentReceipt pr
                    LEFT JOIN Parties p ON p.Id = pr.PartyId
                    GROUP BY pr.VoucherNo
                    ORDER BY pr.Date DESC, pr.VoucherNo DESC", con);
        ...
        private void BtnLoadEdit_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    date = r["Date"].ToString()[..10];
                    var item = new PaymentReceiptItem
                    {
                        SrNo = srNo++,
                ...
                DtPaymentDate.SelectedDate = DateTime.TryParse(date, out var dt) ? dt : DateTime.Today;
        ...
        private void BtnPrintSaved_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                var items = new List<PaymentReceiptItem>();
        ...
        private void BtnDeleteSaved_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                using var con = new SQLiteConnection(_dbPath);
                con.Open();
                var cmd = new SQLiteCommand("DELETE FROM PaymentReceipt WHERE VoucherNo=@v", con);
                cmd.Parameters.AddWithValue("@v", sel.VoucherNo);
                cmd.ExecuteNonQuery();
                LoadReceiptList();
            }
        ...
        private FlowDocument BuildFlowDocument(
            ...
            List<PaymentReceiptItem> items, decimal totalAmount)
        {
            ...
            info.Inlines.Add(new Run($"Payment Receipt #: {voucherNo}    Date: {receiptDate}") { FontWeight = FontWeights.Bold });
            doc.Blocks.Add(info);
            ...
            ph.Child = new TextBlock { Text = "Paid To:", Foreground = Brushes.White, FontWeight = FontWeights.Bold };
            pp.Children.Add(ph);
            ...
            totals.Inlines.Add(new Run($"Total Payment: {totalAmount:N2}") { FontWeight = FontWeights.Bold, FontSize = 14 });
        ...
        // ── Model Classes ─────────────────────────────────────────────────────────
        public class PaymentReceiptItem : System.ComponentModel.INotifyPropertyChanged
        {
            private int _srNo;
